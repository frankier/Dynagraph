...
void emitChanges(std::ostream &os,ChangeQueue<Layout> &Q,const char *view) {
	...
	for(typename Layout::node_iter ni = Q.insN.nodes().begin(); ni!=Q.insN.nodes().end(); ++ni) {
		os << "insert node " << view << " " << mquote(gd<Name>(*ni).c_str())
			<< " " << gd<StrAttrs2>(*ni) << std::endl;
		gd<StrAttrChanges>(*ni).clear();
	}
	for(typename Layout::graphedge_iter ei = Q.insE.edges().begin(); ei!=Q.insE.edges().end(); ++ei) {
	...
	for(typename Layout::node_iter ni = Q.client->nodes().begin(); ni!=Q.client->nodes().end(); ++ni)
		if(!gd<StrAttrChanges>(*ni).empty()) {
			os << "modify node " << view << " " << mquote(gd<Name>(*ni).c_str())
				<< " " << gd<StrAttrs2>(*ni) << std::endl;
			gd<StrAttrChanges>(*ni).clear();
		}
	for(typename Layout::graphedge_iter ei = Q.client->edges().begin(); ei!=Q.client->edges().end(); ++ei)
